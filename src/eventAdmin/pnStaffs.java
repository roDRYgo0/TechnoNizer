...
public class pnStaffs extends javax.swing.JPanel {

    int idEvent;
    
    public pnStaffs(int idEvent) {
        initComponents();
    ...
    void load(){
        showStaffs owner = new showStaffs(true, idEvent);
        ...
        if(classEvent.eventosShow.get(idEvent).getStaffs() != null){
            for(int i = 0; i < classEvent.eventosShow.get(idEvent).getStaffs().size(); i++){
                showStaffs sS = new showStaffs(false, idEvent, i);
                sS.setSize(370, 45);
